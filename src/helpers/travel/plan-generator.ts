...
const SYSTEM_PROMPT = `You turn travel-video transcripts into practical, day-by-day itineraries.

You receive an auto-generated transcript with [mm:ss] timestamps, plus the video's chapter list when it has one. Auto-captions are noisy: place names are often misspelled and sentences run together. Infer the intended names.
      ...
      "places": [
        {
          ...
          "timeHint": "<e.g. 'Morning', 'After sunset'>",
...
Rules:
- Only include places the transcript actually supports. Never invent landmarks to pad a day.
- 2 to 5 days, 2 to 5 places per day, 2 to 4 tips.
- Set sourceTimestamp from the transcript whenever you can — it is how the user checks your work.
- Order days as a route someone could actually walk or drive.
- Write every user-facing string in ${PLAN_LANGUAGE}.`;

// -- Prompt input -------------------------------------------------------------

function buildDigest(transcript: VideoTranscript): string {
  ...
  const lines: string[] = [];
  let budget = MAX_DIGEST_CHARS - chapterBlock.length;
  ...
  return `${chapterBlock}TRANSCRIPT\n${lines.join('\n')}`;
}
  ...
  return value
    .map((entry): PlanPlace | null => {
      ...
      return {
        ...
        sourceTimestamp: optionalStr(record.sourceTimestamp),
      };
    })
